...
set_cipher_data(struct sess_ctx *s_ctx, 
  ...
  //printk(KERN_ERR "set_cipher_data: s_ctx: %p key_size: %d\n", s_ctx, psessdata->cipherSetupData.cipherKeyLenInBytes);
  ...
  //printk(KERN_ERR "Returning from set_cipher_data: cipher_data: %p cipher_len: %d\n", s_ctx->cipher_data, s_ctx->cipher_data->cipherKeyLenInBytes);
...
cpaCySymPerformOp(const CpaInstanceHandle instanceHandle,
  ...
  //printk(KERN_ERR "performop almost done \n");
  app_req.callbk_tag = lreq;

  //printk(KERN_ERR "Calling ixp_queue_request %d %d %d\n", app_req.iv_size, s_ctx->cipher_data->cipherKeyLenInBytes, app_req.msg_size);
